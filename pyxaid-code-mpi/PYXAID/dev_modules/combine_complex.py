    ...
    for i in range(0,sz):
        ...
        for j in range(0,sz):
            if i==j:
                ...
                line = line + "%15.10f " % ((-1.0/13.60569253) * H_re[i][2*j+1] )
        f.write(line+"\n")
    f.close()
    ...
    for i in range(0,sz):
        ...
        for j in range(0,sz):
            if i==j:
                ...
                line = line + "%15.10f " % ((-1.0/13.60569253) * H_re[i][2*j])
        f.write(line+"\n")
    f.close()
...
# res_dir - is the place where resulting 1-line energy files will be written
    ...
    while i < imin + sz:
        print(i)
        combine_old_VASP("%senergy%i" % (res_dir,i), "%sreal%04d" % (res_dir,i), "%s0_Ham_%i_re" % (res_dir,i), "%s0_Ham_%i_im" % (res_dir,i))
        i = i + 1


# Utilization examples
# This is for users of older versions of PYXAID (probably none except me :) )
# To run this example, uncomment the following line!!!
#combine_all_QE(0,1578,"res/")

# This is for users of loder VASP-based scripts
# The "energy" file is in current directory (where from this script is started)
# The couplings "real" files are in "REAL_1000_1200" sub-directory of the current directory
# 1001 - will be the minimal index used in construction of Ham_ files (which will also be
# stores in "REAL_1000_1200" directory - you may want to move them to some other place after).
# To run this example, uncomment the following line!!!
#combine_all_VASP("energy","REAL_1000_1200/",1001)
#combine_all_VASP("energy","./",0002)
combine_all_VASP("../combine/energy_2_1548","../REAL_1_1548/",2)
