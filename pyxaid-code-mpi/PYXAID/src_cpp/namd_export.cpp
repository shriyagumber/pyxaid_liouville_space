...
//>>>>>>>>>>>>>>>>>>>>>>>> INITIALIZATION PART <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  srand(time(0));
  //srand(0);   // For debugging only, here and in the icond loop
  ...
  // H_ij = -i*F_ij   if i!=j (imaginary, off-diagonal, "-" is the convention implying that F_ij = hbar * <i|d/dt|j> )
    ...
    for(int j=0;j<max_indx;j++){
      ...
      int j1;
      j1=j%(params.Ham_ed_indx-params.Ham_st_indx+1)+params.Ham_st_indx;
      std::string Ham_re_file; Ham_re_file = params.Ham_re_prefix + int2string(j1) + params.Ham_re_suffix;
...
//      cout<<"Ham_re(cropped):"<<endl; show_2D(Ham_re_crop);
      ...
      std::string Ham_im_file; Ham_im_file = params.Ham_im_prefix + int2string(j1) + params.Ham_im_suffix;
...
//      cout<<"Composed Ham = "<<Ham<<endl;
      ...
      if(params.is_field==1){
        ...
        std::string Hprime_x_file; Hprime_x_file = params.Hprime_x_prefix + int2string(j1) + params.Hprime_x_suffix;
        std::string Hprime_y_file; Hprime_y_file = params.Hprime_y_prefix + int2string(j1) + params.Hprime_y_suffix;
        std::string Hprime_z_file; Hprime_z_file = params.Hprime_z_prefix + int2string(j1) + params.Hprime_z_suffix;
...
//>>>>>>>>>>>>>>>>>>>>>>>>> MAIN PROGRAM PART <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  for( icond=params.myproc; icond<iconds.size(); icond += params.nprocs){
     //srand(icond);   // For debugging only
