...
//  if(is_many_electron_algorithm){ cout<<"many_electron_algorithm = "<<many_electron_algorithm<<endl; }
  ...
  if(is_Ham_st_indx){ cout<<"Ham_st_indx = "<<Ham_st_indx<<endl;}
  if(is_Ham_ed_indx){ cout<<"Ham_ed_indx = "<<Ham_ed_indx<<endl;}
...
//  if(!is_overlap_re_suffix){ warning("overlap_re_suffix","_re"); overlap_re_suffix = "_re";  is_overlap_re_suffix = 1; }
  ...
  if(!is_Ham_st_indx){ warning("Ham_st_indx","0"); Ham_st_indx = 0; is_Ham_st_indx = 1; wrn_status++;}
  if(!is_Ham_ed_indx){ warning("Ham_ed_indx","-1"); Ham_ed_indx = -1; is_Ham_ed_indx = 1; wrn_status++;}
...
//    else if(s1=="many_electron_algorithm"){ many_electron_algorithm = extract<int>(params[s1]); is_many_electron_algorithm = 1; }
    ...
    else if(s1=="Ham_st_indx"){ Ham_st_indx = extract<int>(params[s1]); is_Ham_st_indx = 1;}
    else if(s1=="Ham_ed_indx"){ Ham_ed_indx = extract<int>(params[s1]); is_Ham_ed_indx = 1;}
...
void InputStructure::sanity_check(){
  //file index control
  if (is_Ham_ed_indx==0 || (Ham_ed_indx - Ham_st_indx)<=0) {
    cout<<"Error: Ham_st_indx = "<<Ham_st_indx<<", Ham_ed_indx = "<<Ham_ed_indx<<endl;
    cout<<"Allowed values are:\n";
    cout<<"        Ham_st_indx>=0, default 0 if not set\n";
    cout<<"        Ham_ed_indx>Ham_st_indx\n";
    cout<<"Exiting...\n";
    exit(0);
  }
